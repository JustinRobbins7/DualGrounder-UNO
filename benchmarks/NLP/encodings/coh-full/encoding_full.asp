...
% COH objective (Coherence Maximality)

% we reach the goal from the goal
reach(P,P) :- goal(P).

% if we reach P and infer P (inferVia(R,P), inferenceNeeds(P,R,Q))
% -> we reach Q
reach(Q,From) :- reach(P,From), inferVia(R,P), inferenceNeeds(P,R,Q).

% if we reach P/S/O and factor P/S/O from P/SS/OO that was inferred or abduced -> we reach P/SS/OO
reach(Q,From) :- reach(P,From), factorVia(P,Q).

% to maximize coherence, minimize non-reachable pairs
reachFromBoth(P,Q) :- goal(P), goal(Q), P < Q, reach(N,P), reach(N,Q).
:~ goal(P), goal(Q), P < Q, not reachFromBoth(P,Q). [1@1,P,Q]

%
% == modules/acyc-trans/eq.lp ==
%

% ensuring transitivity of the eq/2 relation in pure ASP
:- eq(A,B), eq(B,C), A != B, B != C, A != C, not eq(A,C).
